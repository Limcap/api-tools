...
namespace Limcap.ApiTools {
	...
	public class DataCollection : IEnumerable<KeyValuePair<string, object>> {
		...
		public DataCollection( IDictionary<string, object> source = null ) {
			dict = source ?? new Dictionary<string, object>();
		}

		public DataCollection( params KeyValuePair<string, object>[] items ) {
			dict = new Dictionary<string, object>();
			foreach (var item in items) dict.Add(item);
		}

		public DataCollection( params (string key, object value)[] items ) {
			dict = new Dictionary<string, object>();
		...
		public string ToJson() {
			var opt = new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore };
			var serialized = JsonConvert.SerializeObject(dict, opt);
		...
		public static implicit operator DataCollection( Dictionary<string, object> source ) {
			return new DataCollection(source);
		}
