...
namespace StandardApiTools
{
    public static class StdApiUtil
    {
        ...
        public static string AsString(this byte[] bytes, Encoding charset = null)
        {
            charset = charset != null ? charset : Encoding.Default;
            var rs = new MemoryStream(bytes);
            StreamReader sr = charset != null
                ? new StreamReader(rs, charset)
        ...
        public static string AddRename<T>(this Dictionary<string, T> dict, string key, T value)
        {
            ...
            string RenameKey(string key2, Dictionary<string, T> dict2)
            {
                var newkey2 = key2;
                int keyCount = 2;
                while (dict2.ContainsKey(newkey2))
                {
                    if (keyCount == int.MaxValue) { key2 += "_"; keyCount = 1; }
                    newkey2 = $"{key2}({keyCount++})";
                }
                return newkey2;
            }
        }
